    ...
    function renderSubMenu(tab: TPageTab) {
        return tab.submenu?.map((sm) => {
            return (
                ...
                >
                    {tab.submenu && (
                        <ul className="rounded-sm flex flex-col gap-1 absolute border border-base-100 bg-base-200 w-52 p-2 h-auto left-full -top-2">
                            {renderSubMenu(sm)}
                        </ul>
                    )}
                </Submenu>
    ...
    return (
        ...
        >
            ...
            {tab.submenu && showMenu && (
                <ul className="rounded-tr-sm z-10 rounded-br-sm rounded-bl-sm flex flex-col gap-1 absolute bg-base-200 w-52 h-auto p-2 left-0 text-[9pt] border border-base-100">
                    {renderSubMenu(tab)}
                </ul>
            )}
        </li>
